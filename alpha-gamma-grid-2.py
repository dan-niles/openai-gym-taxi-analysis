...
if __name__ == "__main__":
    ...
    df.rename(columns = {0:'Alpha'}, inplace = True)
    df.rename(columns = {1:'Gamma'}, inplace = True)
    df.rename(columns = {2:'Episode Number'}, inplace = True)
    df.rename(columns = {3:'Cumulative Reward'}, inplace = True)

    sns.lmplot(x='Episode Number', y='Cumulative Reward', data=df, row='Alpha', col='Gamma',order=9, scatter=False)
    plt.show()